...
function buildCustomSectionDOM(def){
  ...
  page.innerHTML = `
    ...
    ${isDeduction ? `<div class="field-hint" style="margin-bottom:12px">⬇️ This is a <strong>Deduction section</strong> — amounts entered here are deducted from your Wallet Balance.</div>` : ''}
...
function renderCustomSection(defId){
  ...
  // Auto S.No for deduction sections
  const isDeduction = def.sectionType === 'deduction';
  const allRows = db.custom[defId] || [];
  const amtCols = def.columns.filter(c=>c.type==='number');
  const total = allRows.reduce((s,r)=>s+amtCols.reduce((a,col)=>a+(parseFloat(r[col.key])||0),0),0);

  const kpiEl = document.getElementById(`kpi-custom-${defId}`);
  if(kpiEl){
    kpiEl.innerHTML = skpi('Entries', allRows.length, 'b') +
      amtCols.map(col => skpi(col.label, fmt(allRows.reduce((s,r)=>s+(parseFloat(r[col.key])||0),0)), isDeduction?'r':'g')).join('');
  }

  const tbody = document.querySelector(`#tbl-custom-${defId} tbody`);
  if(!tbody) return;
  if(!rows.length){
    tbody.innerHTML = `<tr class="empty-row"><td colspan="${def.columns.length + (isDeduction?2:1)}">No entries yet. Click + Add Entry to start.</td></tr>`;
    return;
  }
...
function customSectionsSummaryRows(){
  ...
  return loadCustomDefs().map(def=>{
    ...
    return { label:def.icon+' '+def.name, count:active.length, amount:total, isDeduction };
  });
}
